        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new BlogUser
                {
                    ...
                    Email = Input.Email,
                    ImageData = (await _imageService.EndcodeImageAsync(Input.ImageFile)) ??
                                 await _imageService.EndcodeImageAsync(_configuration["DefaultUserImage"]),
                    ContentType = Input.ImageFile is null ?
                                    Path.GetExtension(_configuration["DefaultUserImage"]):
                                    _imageService.ConetentType(Input.ImageFile)
                };
